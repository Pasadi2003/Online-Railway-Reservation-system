...
Class Master extends DBConnection {
	...
	function save_reservation(){
		...
		extract($_POST);
		$capacity = $this->conn->query("SELECT `".($seat_type == 1 ? "first_class_capacity" : "second_class_capacity")."` FROM train_list where id in (SELECT train_id FROM `schedule_list` where id ='{$schedule_id}') ")->fetch_array()[0];
